...
> Diese Datei dokumentiert die separate Python-Referenzengine unter `tools/` und `engine/`. Der aktuelle lokale Produkt-Slice der Web-App liegt in `app/optimizer.mjs`; seinen tatsächlichen Stand beschreibt [.agent/CONTINUITY.md](../.agent/CONTINUITY.md). Beide Wege verwenden die kanonischen Daten, sind aber nicht dieselbe Implementierung.
