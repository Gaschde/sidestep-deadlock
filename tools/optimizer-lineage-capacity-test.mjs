...
function runGroup(rt, seedPaths, width, horizon) {
  ...
  while (reached && queue.length && !cap) { const candidates = []; for (const node of queue) { for (const candidate of expand(node, (x) => x.state.earnedSouls === horizon && x.event?.type !== "save")) { maxDepth = Math.max(maxDepth, candidate.depth); if (!closure.has(closureKey(candidate))) candidates.push(candidate); } if (cap) break; } if (!candidates.length) { closureComplete = !cap; break; } const unique = dedupeFuturePathHistory(candidates, rt.futureKey, vector); const next = []; for (const node of unique) { const key = closureKey(node); if (!closure.has(key)) { closure.set(key, node); next.push(node); } } if (!next.length) { closureComplete = !cap; break; } queue = next; }
