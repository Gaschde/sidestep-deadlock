...
function renderPhase() {
  ...
  const spiritSummary = $("#damage-focus").value === "weapon"
    ? `<p><strong>Weapon-Mechaniken:</strong> Nur die Waffe und permanente Itemwerte zählen in den Schaden. Bekannte aktive Fähigkeitsdaten (${abilityNames}) werden angezeigt, aber nicht als Weapon-Schaden angerechnet. ${knownGaps}</p>`
    : `<p><strong>${focusLabel}-Mechaniken:</strong> ${abilityNames}. ${knownGaps}</p>`;
  const afterburn = combat.afterburn_mechanics;
  const afterburnSummary = afterburn?.applicable
    ? `<p><strong>Afterburn:</strong> ${afterburn.included ? `Bei ${afterburn.triggerHits} belegten Weapon-Hits auf dasselbe Heldenziel entsteht der Brand; im 10-s-Fenster trägt er ${teamfight?.afterburn_damage?.toFixed(1) || "0.0"} Schaden bei. ${focusLabel === "Spirit" ? "Das reine Spirit-Profil erzeugt keine Weapon-Hits und rechnet deshalb keinen Afterburn-Schaden." : "Die Weapon-/Hybrid-Bewertung zählt ihn getrennt vom direkten Fähigkeitsschaden."}` : afterburn.treatment} ${afterburn.treatment}</p>`
    : "";
